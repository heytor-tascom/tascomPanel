...
<body id="page-index">
    ...
    <div class="row">
        <div class="col-md-2">@include('integracao.header.index', ['custoActive' => 'active'])</div>
        <div class="col-md-10">
                            ...
                            <table class="table table-sm table-hovered">
                                ...
                                <tbody>
                                    ...
                                    @forelse ($messages as $message)
                                        ...
                                        <tr>
                                            <td> {{ $message->cd_atendimento }} </td>
                                            <td> {{ date('d/m/Y', strtotime($message->updated_at)) }} </td>
                                            <td>
                                                @if ($message->sn_integrado == 'S')
                                                    <p class="text-success">INTEGRADO</p>
                                                @endif
                                                @if ($message->sn_integrado == 'G')
                                                    <p class="text-info">XML GERADO</p>
                                                @endif
                                                @if ($message->sn_integrado == 'F')
                                                    <p class="text-danger">ERRO</p>
                                                @endif
                                                @if ($message->sn_integrado == 'N')
                                                    <p class="text-info">AGUARDANDO</p>
                                                @endif
                                            </td>
                                            <td>
                                                <a href="#" class="button white edit" title="Ver informações"
                                                    style="background-color:#eaeaea; color:#5a67a9;"
                                                    data-bs-toggle="modal" data-bs-target="#exampleModal"
                                                    onclick="getReturnDRG({{ $message->id }})">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <button class="delete button" title="Ver XML" data-bs-toggle="modal"
                                                    data-bs-target="#exampleModal"
                                                    onclick="getReturnXML({{ $message->id }})">
                                                    <i class="fas fa-search" style="color: dimgrey"></i>
                                                </button>
                                                {{-- <button class="delete button" title="Regerar XML">
    ...
    <div class="modal fade" id="exampleModal" style="color:black" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
