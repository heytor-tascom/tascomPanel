...
@section('content')
    @include('layouts.rhp.navbars.farmacia.centralV2')
            ...
            <div class="card-body px-0 pt-4">
                ...
                <table class="table table-sm" id="tableSolicitacoes">
                    ...
                    <tbody>
                        ...
                        @inject('helperSolSaiPro', 'App\Http\Helpers\SolSaiProHelpers')
                        ...
                        @forelse ($lista as $toda)
                            ...
                            <tr>
                                <td class="text-center {{ $classLinha }}" style="font-size:1.8em;">
                                    <i class="fas fa-exclamation-triangle"></i>
                                    @if (isset($toda->dt_alta_medica) && !is_null($toda->dt_alta_medica))
                                    &nbsp;<i class="fas fa-user-check" style="color:#00e089;"></i>
                                    @endif
                                </td>
                                <td class="text-center {{ $classLinha }}">{{$toda->cd_solsai_pro}}</td>
                                <td class="{{ $classLinha }}">{{ $toda->ds_leito }}</td>
                                <td class="text-center {{ $classLinha }}">{{ $toda->cd_atendimento }}</td>
                                <td class="{{ $classLinha }}">{{ $toda->nm_paciente }}</td>
                                <td class="text-center {{ $classLinha }}">{{ $toda->dt_nascimento }}</td>
                                <td class="{{ $classLinha }}">{{ $toda->nm_setor }}</td>
                                <td class="text-center" style="font-size:1.8em;">
                                    {!! $helperSolSaiPro::overdue( $toda->pri_necessidade ?? null, $toda->dt_hr_solsai_pro, $toda->tp_origem_solicitacao, $toda->sn_urgente, $toda->sn_atendida ?? '') !!}
                                ...
                                <td class="text-center {{ $classLinha }}">{{ $toda->pri_necessidade_view ?? $toda->dt_hr_solsai_pro_view  }}</td>
                                @if ($aba == 'avulsas' || $aba == 'devolucoes')

                                @else
                                    <td class="{{ $classLinha }}">{{ $toda->ds_turno }}</td>
                                @endif
...
<script>
    ...
    $('.srcClass').click(function(){
        $('.srcClass').val(null);
        searchPatient(4, null);
    });

    function searchPatient(col, par) {
        var input, filter, table, tr, td, i, txtValue;

        if (par == null){
            filter = '';
        }else{
            input = document.getElementById(par.id);
