...
namespace App\Http\Controllers\Home\Produtos\Paineis\Enfermagem\dev;
...
class ChecagemController extends Controller
{
    public function index(Request $request)
    {
        ...
        $title              = "PAINEL DE CHECAGEM HOMOLOGAÇÃO";
        $filterSetores      = [];
        ...
        if (isset($params['tipoVisualizacao']) && $params['tipoVisualizacao'] == 'd8375b48751caf44e5c23d4b0dcc2984d6081784') {
            ...
            $title            = "GESTÃO DO PAINEL DE CHECAGEM HOMOLOGAÇÃO";
        }
        ...
        $atendimentos  = Atendimento::join("leito", "leito.cd_leito", "=", "atendime.cd_leito")
                                ...
                                ->orderBy("setor.nm_setor", "ASC")
                                ->orderBy("leito.ds_leito", "ASC")
                                ->get();
        ...
        if  (count($atendimentos) == 0){ 
            ...
            $atendimentos  = Atendimento::join("ori_ate", "ori_ate.cd_ori_ate", "=", "atendime.cd_ori_ate")
                                ->join("setor", "setor.cd_setor", "=", "ori_ate.cd_setor")
                                ->whereNull("atendime.dt_alta")
                                ...
                                ->select("atendime.cd_atendimento", "atendime.cd_paciente", "atendime.dt_atendimento", "setor.nm_setor")
                                ->orderBy("setor.nm_setor", "ASC")
                                ->get();
        ...
        return view("home.produtos.paineis.enfermagem.checagem.index", compact("title", "atendimentos", "setores", "tipoVisualizacao", "refreshTime"));
    }
                                    ...
                                    ->with([
                                        ...
                                        "itens" => function($query) {
                                            $query->join("tip_presc", "tip_presc.cd_tip_presc", "=", "itpre_med.cd_tip_presc")
                                                // ->rightJoin("tip_fre", "tip_fre.cd_tip_fre", "=", "itpre_med.cd_tip_fre")
                                                ...
                                                ->select("cd_pre_med", "cd_itpre_med", "tip_presc.cd_tip_presc", "sn_cancelado", "dh_cancelado", "sn_urgente");
                                        }
                                    ])
    ...
    public static function horariosItemPrescricao($cdItemPrescricao)
    {
        return DB::connection('oracle')
                    ->select("SELECT  hritpre_med.cd_itpre_med,
                                    ...
                                    (
                                        SELECT	TO_CHAR(Max(ie.DT_GRAVACAO),'DD/MM/RRRR HH24:MI:SS')
                                        FROM 	SOLSAI_PRO s,
                                                ITSOLSAI_PRO its,
                                                ITMVTO_ESTOQUE ie
                                        WHERE 	s.CD_SOLSAI_PRO 	= its.CD_SOLSAI_PRO
                                        AND 	its.CD_ITSOLSAI_PRO = ie.CD_ITSOLSAI_PRO
                                        AND 	its.CD_ITPRE_MED	= ITPRE_MED.CD_ITPRE_MED
                                    ) as dh_devolucao,
                                    (
                                        SELECT	TO_CHAR(Max(mve.HR_ENTREGA),'DD/MM/RRRR HH24:MI:SS')
                                        FROM 	SOLSAI_PRO s,
                                                ITSOLSAI_PRO its,
                                                MVTO_ESTOQUE mve,
                                                ITMVTO_ESTOQUE ie
                                        WHERE 	s.CD_SOLSAI_PRO 	= its.CD_SOLSAI_PRO
                                        AND 	its.CD_ITSOLSAI_PRO = ie.CD_ITSOLSAI_PRO
                                        AND  	ie.CD_MVTO_ESTOQUE	= MVE.CD_MVTO_ESTOQUE
                                        AND 	mve.CD_SOLSAI_PRO 	= s.CD_SOLSAI_PRO
                                        AND 	its.CD_ITPRE_MED	= ITPRE_MED.CD_ITPRE_MED
                                    ) as dh_recebimento,
                                    -- (
    ...
    public static function getStatusPacientes($cdAtendimento)
    {
        return DB::connection('oracle')
                    ...
                    COUNT(CASE WHEN tempo_prox_minutos_med >= 0 THEN 1 END) AS total_ate_1h_med,
                    COUNT(CASE WHEN tempo_prox_minutos_med < 0 THEN 1 END) AS total_atrasado_med,
            ...
            FROM 	(
                SELECT  pre_med.cd_atendimento,
                        NULL AS tempo_prox_minutos_med,
                        CASE
                            WHEN dh_checagem IS NULL THEN round((hritpre_med.dh_medicacao - SYSDATE)*1440)
                        ELSE NULL
                        END
                ...
                AND     tip_presc.cd_tip_esq            NOT IN ('EXL', 'EXI')
                ...
                SELECT  pre_med.cd_atendimento,
                        ...
                        ELSE NULL
                        END
                        AS tempo_prox_minutos_med,
                        NULL AS tempo_prox_minutos,
                        (
                        SELECT  TO_CHAR(Max(DH_MODIFICACAO),'DD/MM/RRRR HH24:MI:SS')
                        FROM    LOG_HRITPRE_MED
                        WHERE   CD_ITPRE_MED      = ITPRE_MED.CD_ITPRE_MED
