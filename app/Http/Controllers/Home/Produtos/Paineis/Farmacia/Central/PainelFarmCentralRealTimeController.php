...
class PainelFarmCentralRealTimeController extends Controller
{
        ...
        return DB::connection('oracle')
                    ->select(
                       ...
                       -- AND Trunc(solsai_pro.dt_solsai_pro) >= Trunc(SYSDATE)
					    AND (
                              Trunc(solsai_pro.dt_solsai_pro) <= Trunc(SYSDATE-1)
                              and 
                              Trunc(solsai_pro.dt_solsai_pro) <= Trunc(SYSDATE+1)                              
                            )  
    ...
    public static function transferencias($parSetores, $parEstoque)
    {
        ...
        return DB::connection('oracle')
                    ->select(
                        ...
                        WHERE  SOLSAI_PRO.CD_ESTOQUE = 16
                        AND SOLSAI_PRO.TP_SITUACAO IN ( 'P')
        ...
        return DB::connection('oracle')
                    ->select(
                        "SELECT * FROM (
                            ...
                            AND solsai_pro.cd_estoque = $parEstoque
                            AND solsai_pro.tp_situacao IN ( 'P' )
                            AND solsai_pro.tp_solsai_pro IN ( 'P', 'E', 'C' , 'S' )
